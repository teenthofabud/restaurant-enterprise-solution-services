package com.teenthofabud.restaurant.solution.reservation.category.controller;
...
import com.teenthofabud.restaurant.solution.reservation.category.data.CategoryMessageTemplate;
...
@Slf4j
@Tag(name = "Category API", description = "Manage dining categorys and their details")
public class CategoryController {
    ...
    public Set<CategoryVo> getAllCategoryNaturallyOrdered() {
        log.debug("Requesting all available categorys by their natural orders");
        Set<CategoryVo> naturallyOrderedCategorys = service.retrieveAllByNaturalOrdering();
        log.debug("Responding with all available categorys by their natural orders");
        return naturallyOrderedCategorys;
    }
    ...
    @ResponseStatus(HttpStatus.OK)
    @GetMapping("filter")
    public List<CategoryVo> getAllCategorysByFilters(@RequestParam(required = false) String name,
                                                       @RequestParam(required = false) String description) throws CategoryException {
        log.debug("Requesting all available categorys with given filters");
        ...
        if(!emptyName || !emptyDescription) {
            ...
            log.debug("Responding with all available categorys with given filters");
            return matchedByFilter;
        }
        log.debug("category filters are empty");
        throw new CategoryException(ReservationErrorCode.RESERVATION_ATTRIBUTE_INVALID, new Object[] { "filters" });
    }
    ...
    @io.swagger.v3.oas.annotations.Parameter(in = ParameterIn.QUERY, description = "levels of nested fields to be unfolded within the response body")
        ...
        if(StringUtils.hasText(StringUtils.trimWhitespace(id)) && StringUtils.isEmpty(StringUtils.trimWhitespace(cascadeUntilLevel))) {
            ...
            try {
                ...
                throw new CategoryException(ReservationErrorCode.RESERVATION_ATTRIBUTE_INVALID, new Object[] { "cascadeUntilLevel", cascadeUntilLevel });
            }
        }
        log.debug(CategoryMessageTemplate.MSG_TEMPLATE_CATEGORY_ID_EMPTY.getValue());
        throw new CategoryException(ReservationErrorCode.RESERVATION_ATTRIBUTE_INVALID, new Object[] { "id", id });
