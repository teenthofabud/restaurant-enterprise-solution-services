...
public interface CheckInAPI<T extends CheckInForm, U extends CheckInVo> {
    ...
    @Operation(summary = "Get all CheckIn details by sequence on date")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Retrieve all available CheckInes and their details that match the given category id",
                    content = { @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, array = @ArraySchema(schema =
    ...
    public List<U> getAllCheckInsBySequenceOnDate(@Parameter(in = ParameterIn.PATH, name = "sequence",
            description = "sequence number for a day") String sequence,
                                                          @Parameter(in = ParameterIn.QUERY, name = "date",
            description = "expected date the sequence was generated")  String date) throws CheckInException;
