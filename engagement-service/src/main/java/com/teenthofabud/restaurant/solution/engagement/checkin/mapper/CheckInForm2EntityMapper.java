...
public abstract class CheckInForm2EntityMapper<T extends CheckInEntity, U extends CheckInForm> implements DualChannelMapper<T, U> {
    ...
    /*@Override
    public Optional<T> compareAndMap(T actualEntity, U form) {
        ...
        *//*if(!fieldsToEscape.contains("status") && StringUtils.hasText(StringUtils.trimWhitespace(form.getStatus()))) {
        ...
        if(changeSW) {
            Optional<T> optionalExpectedEntity = this.compareAndMap(expectedEntity, (T) actualEntity, (U) form);
    ...
    //protected abstract Optional<T> compareAndMap(CheckInEntity parent, T checkInEntityChild, U checkInFormChild);
