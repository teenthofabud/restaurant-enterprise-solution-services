...
public class EngagementVo extends TOABBaseVo implements Comparable<EngagementVo> {
    ...
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "dd-MM-yyyy") // date pattern referenced from res.session.engagement.event.date.format
    @ToString.Include
    private LocalDate date;
    @JsonSerialize(using = LocalDateSerializer.class)
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "HH:mm:ss") // date pattern referenced from res.session.engagement.event.time.format
    @ToString.Include
    private LocalTime time;
    @JsonSerialize(using = LocalDateSerializer.class)
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "dd-MM-yyyy HH:mm:ss") // date pattern referenced from res.session.engagement.event.timestamp.format
    @ToString.Include
